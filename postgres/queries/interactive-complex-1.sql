select
  ...
  p1.pl_name,
  (select array_agg(ARRAY[o2.o_name, pu_classyear::text, p2.pl_name] order by o2.o_organisationid) from person_university, organisation o2, place p2  where pu_personid = id and pu_organisationid = o2.o_organisationid and o2.o_placeid = p2.pl_placeid group by pu_personid) as university,
       (select array_agg(ARRAY[o3.o_name, pc_workfrom::text, p3.pl_name] order by o3.o_organisationid) from person_company, organisation o3, place p3 where pc_personid = id and pc_organisationid = o3.o_organisationid and o3.o_placeid = p3.pl_placeid group by pc_personid) as company
from
    (
